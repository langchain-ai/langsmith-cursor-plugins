...
var FEEDBACK_COMMAND_PREFIXES = ["/langsmith", "/ls"];
var FEEDBACK_KEY = "user_feedback";
...
var Client = class _Client {
  ...
  static getDefaultClientConfig() {
    ...
    const apiUrl = getLangSmithEnvironmentVariable("ENDPOINT") ?? DEFAULT_API_URL2;
...
async function submitFeedback(target, payload) {
  ...
  for (const c of clients) {
    try {
      ...
      warn(`Feedback post failed for run ${target.runId}: ${err}`);
    }
  }
  return posted;
}
...
// dist/feedback.js
var SENTIMENTS = [
  { words: ["good", "great", "\u{1F44D}", "+1", "up", "yes", "positive"], label: "\u{1F44D} good", fields: { score: 1 } },
  { words: ["bad", "poor", "\u{1F44E}", "-1", "down", "no", "negative"], label: "\u{1F44E} bad", fields: { score: 0 } },
  {
    words: ["flag", "bookmark", "revisit", "todo", "star", "\u2B50", "mark"],
    label: "\u{1F6A9} flagged",
...
function stripPrefix(prompt) {
  ...
  if (!FEEDBACK_COMMAND_PREFIXES.includes(head))
    return null;
  return firstSpace === -1 ? "" : trimmed.slice(firstSpace + 1).trim();
}
function parseFeedbackCommand(prompt) {
  ...
  return { kind: "feedback", key: FEEDBACK_KEY, label: "\u{1F4DD} note", comment: rest };
}
function feedbackHelpText() {
  return [
    "LangSmith feedback \u2014 flags the trace of your previous turn:",
    "  /langsmith good [comment]   \u{1F44D} score this turn",
    "  /langsmith bad [comment]    \u{1F44E} score this turn",
    "  /langsmith flag [comment]   \u{1F6A9} bookmark to revisit",
    "  /langsmith <note>           \u{1F4DD} attach a comment"
  ].join("\n");
}
...
async function handleFeedbackCommand(config, input, command) {
  ...
  try {
    const posted = await submitFeedback({ runId: lastTrace.runId }, {
      key: command.key,
      score: command.score,
      value: command.value,
