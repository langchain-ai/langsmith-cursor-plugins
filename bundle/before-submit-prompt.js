...
import { closeSync as closeSync3, constants as constants2, fstatSync as fstatSync2, lstatSync as lstatSync2, openSync as openSync3, readFileSync as readFileSync3 } from "node:fs";
...
function readPolicy(path) {
  let raw;
  try {
    const fd = openSync3(path, constants2.O_RDONLY | constants2.O_NONBLOCK | constants2.O_NOFOLLOW);
    try {
...
function getThreadTracingMode(path, sessionId, defaultMuted = false) {
  try {
...
async function handlePromptSubmit(input) {
  ...
  try {
    ...
    await atomicUpdateState(config.stateFilePath, (s) => reduceBeforeSubmitPrompt(s, input, Date.now(), enabled ? getThreadTracingMode(tracingPolicyPath(), input.conversation_id, config.defaultMuted) : "off"));
