...
function subagentLaunchMode(conv, input) {
  ...
  const generation = ![
    ...
    input.session_id
  ].includes(input.generation_id) ? conv.turns[input.generation_id] : void 0;
...
function reduceSubagentStart(state, input, nowMs) {
  ...
  const turn = turnId ? conv.turns[turnId] : newTurnBuffer(input.generation_id, nowMs, nextTurnNum(conv));
  turn.subagents.push({
    tracingMode,
