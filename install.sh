...
INSTALLER_VERSION="langsmith-cursor-tracing installer 1.0"
REPOSITORY="langchain-ai/langsmith-cursor-plugins"
EXECUTABLE="langsmith-cursor-tracing"
RELEASE_PAGE_SIZE=100
RELEASES_API="${LANGSMITH_CURSOR_RELEASES_API:-https://api.github.com/repos/$REPOSITORY/releases?per_page=$RELEASE_PAGE_SIZE}"
DOWNLOAD_BASE="${LANGSMITH_CURSOR_DOWNLOAD_BASE:-https://github.com/$REPOSITORY/releases/download}"
...
print_help() {
  cat <<'HELP'
Install the LangSmith tracing binary for Cursor.

Usage:
  curl -LsSf https://langch.in/cursor-tracing | bash
  curl -LsSf https://langch.in/cursor-tracing | bash -s -- [options]
  curl -LsSf https://langch.in/cursor-tracing | bash -s -- VERSION
...
Environment variables:
  LANGSMITH_CURSOR_RELEASES_API    GitHub releases API to install from
