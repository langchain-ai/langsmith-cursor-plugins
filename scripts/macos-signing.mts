...
export async function cscNotarizeMacOS(
  executable: string,
  releaseName: string,
  ...
  try {
    ...
    const apiKeyPath = await materializeSecret(
    ...
    if (expectedTeamId && !identity.endsWith(`(${expectedTeamId})`)) {
      throw new Error("Signing identity does not match APPLE_TEAM_ID embedded at build time");
    }
...
export async function packageRelease(
  ...
  console.log(`Built packaged ${path.basename(releaseExecutable)}`);
}
...
export async function inferMacTeamIdFromSigningCertificate(): Promise<string> {
  let tmpDir: string | undefined;
  let keychain: string | undefined;
  try {
